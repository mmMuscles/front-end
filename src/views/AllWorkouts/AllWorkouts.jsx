    ...
    return (
        ...
        <Link to='/calendar?date='><button>Back To Day</button></Link>
            ...
            {exercises.map((workout) => <Card key={workout.id}> <li key={workout.id}>
                <Workout name={workout.name} description={workout.description} category={workout.category}/>{workouts.includes(workout.id) ? <button className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded' onClick={() => handleRemove(workout.id)}>Remove</button> : <button className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded' onClick={() => handleAdd(workout)}><span>Add</span><span>+</span></button>}
            </li></Card>)}</ul>
            }
        </div>
    )
