...
export default function AllWorkouts() {
  ...
  return (
    <div className="m-14">
      <Link to={`/calendar?date=${date}`}>
        <button className="bg-yellow-700 hover:bg-yellow-700 text-white font-bold py-2 px-4 m-3 rounded">
          Back To Day
        </button>
      </Link>
        ...
        <ul className="all-the-workouts">
          {exercises.map((workout) => (
            <Card key={workout.id}>
              {" "}
              <li key={workout.id}>
                {workouts.includes(workout.id) ? (
                  <button
                    className="bg-red-700 hover:bg-blue-700 text-white font-bold py-2 px-4 mt-1 rounded object-right-bottom"
